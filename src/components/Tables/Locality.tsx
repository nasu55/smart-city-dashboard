...
const LocalityTable = ({ listOfLocalities }: Props) => {
  ...
  const filteredCategories = listOfLocalities?.filter((categoryItem: any) =>
    categoryItem.localityName.toLowerCase().includes(searchTerm.toLowerCase()),
  );
  ...
  async function deleteLocality(id: any) {
    try {
      const responseDelete = await localityApi.deleteLocality(id);
                    ...
                    <tr key={index}>
                      ...
                      >
                        <div className="flex items-center justify-end space-x-3.5">
                          <Link
                            href={`/admin/locality/edit/${packageItem._id}`}
                          >
